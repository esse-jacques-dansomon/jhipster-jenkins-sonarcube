...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.errorListener = eventManager.subscribe('caurisApplicationApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('caurisApplicationApp.httpError', (response: EventWithContent<unknown> | string) => {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName: string = translateService.instant(
                `caurisApplicationApp.${fieldError.objectName as string}.${convertedField}`,
              );
